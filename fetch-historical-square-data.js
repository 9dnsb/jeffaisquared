...
async function fetchComprehensiveData() {
  try {
    ...
    const theWellLocation = locationsResult.locations.find(
      (loc) => loc.id === 'LT8YK4FBNGH17' || loc.name.includes('The Well')
    )
    ...
    if (theWellLocation) {
      ...
      console.log(
        `Orders found (6 months): ${ordersResult.orders?.length || 0}`
      )
      ...
      if (ordersResult.orders && ordersResult.orders.length > 0) {
        ...
        ordersResult.orders.forEach((order) => {
          ...
          dailySales[date] =
            (dailySales[date] || 0) + (order.total_money?.amount || 0) / 100
        })

        console.log(`Total Revenue: $${totalRevenue.toFixed(2)} CAD`)
        console.log(
          `Average Order: $${(
            totalRevenue / ordersResult.orders.length
          ).toFixed(2)}`
        )
        ...
        ordersResult.orders.slice(0, 3).forEach((order, index) => {
          ...
          console.log(
            `  Items: ${
              order.line_items?.map((item) => item.name).join(', ') || 'None'
            }`
          )
        })
        ...
        console.log(
          `Excel has ${936} transactions from April 28 - May 13, 2025`
        )
        console.log(
          `API shows ${ordersResult.orders.length} completed orders in last 6 months`
        )
        console.log(
          `Excel location: LT8YK4FBNGH17 ✅ MATCHES The Well location`
        )
      } else {
      ...
      console.log(
        '❌ The Well location (LT8YK4FBNGH17) not found in current locations'
      )
    }

    // Quick check of all locations
    console.log(`\n📊 ALL LOCATIONS SUMMARY (last 30 days):`)
    for (const location of locationsResult.locations.slice(0, 3)) {
      // Check first 3 to avoid rate limits
      const quickSearch = {
      ...
      try {
        const quickResult = await squareApiRequest(
          '/v2/orders/search',
          'POST',
          quickSearch
        )
        console.log(
          `  ${location.name}: ${quickResult.orders?.length || 0} orders`
        )
      } catch (error) {
